...
export default function ProjectsPage() {
  ...
  const openCreate = () => {
    setEditProject(null);
    setForm({ name: '', description: '', area: '', status: 'active' });
    setShowForm(true);
  };

  const openEdit = (p) => {
    setEditProject(p);
    setForm({ name: p.name, description: p.description || '', area: p.area || '', status: p.status });
    setShowForm(true);
  };
    ...
    <div className="max-w-4xl mx-auto">
      <div className="flex items-center gap-3 mb-6">
        ...
        <div>
          ...
          <p className="text-sm text-gray-500">Project Dashboard — track outcomes</p>
        </div>
      ...
      {loading ? (
        ...
        <div className="grid gap-4 sm:grid-cols-2">
          {projects.map((p) => {
            ...
            return (
              <div key={p.id} className="bg-white rounded-xl border border-gray-200 p-5 hover:shadow-sm transition-shadow group">
                <div className="flex items-start justify-between mb-3">
                  <div>
                    <h3 className="text-sm font-semibold text-gray-900">{p.name}</h3>
                    {p.area && <span className="text-xs text-gray-400">{p.area}</span>}
                  </div>
                  <div className="flex items-center gap-1">
                    <span className={clsx('text-xs px-2 py-0.5 rounded-full font-medium', STATUS_COLORS[p.status])}>
                      {p.status.replace('_', ' ')}
                    </span>
                    <button onClick={() => openEdit(p)} className="p-1 text-gray-300 hover:text-charlie-600 opacity-0 group-hover:opacity-100 transition-all">
                      <Edit2 className="w-3.5 h-3.5" />
                    </button>
                    <button onClick={() => handleDelete(p.id)} className="p-1 text-gray-300 hover:text-red-500 opacity-0 group-hover:opacity-100 transition-all">
            ...
            <form onSubmit={handleSubmit} className="px-6 py-4 space-y-3">
              ...
              <div className="grid grid-cols-2 gap-3">
                <div>
                  ...
                  <input type="text" value={form.area} onChange={(e) => setForm({ ...form, area: e.target.value })} placeholder="Technology, Health..."
                    className="w-full px-3 py-2 border border-gray-200 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-charlie-300" />
                </div>
                <div>
